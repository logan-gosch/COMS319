...
const averagePerProduct = (productName) => {
    let totalPrice = 0;
    let numItems = 0;
    for (let i = 0; i < products.length; i++) {
        if (products[i][1] === productName) {
            totalPrice += products[i][6];
            numItems++;
        }
    }
...
const ceilingFan = averagePerProduct('Ceiling Fan').toFixed(2);;
const wallLight = averagePerProduct('Wall Light').toFixed(2);;
const jacket = averagePerProduct('Jacket').toFixed(2);

console.log('The average price per product is:\n Ceiling Fan: $' + ceilingFan + '\n Wall Light: $' + wallLight + '\n Jacket: $' + jacket);
